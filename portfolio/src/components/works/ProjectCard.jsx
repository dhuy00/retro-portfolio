...
const ProjectCard = () => {
  ...
  return (
    <div className="w-full max-w-[550px] bg-card border-primary/50 border-2 p-4 sm:p-6 flex flex-col gap-4 hover:border-accent transition hover:shadow-[0_0_20px_rgba(0,255,255,0.4)]">
      ...
      <span className="text-muted-foreground font-orbitron text-sm sm:text-base">
      ...
      <div className="flex gap-2 sm:gap-4 flex-wrap">
        {techstack.map((item) => (
          <span
            key={item.id}
            className="font-orbitron border border-accent/50 text-accent font-medium px-2 py-1 text-xs sm:text-sm"
          >
            {item.name}
          </span>
        ))}
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between gap-4 sm:items-center">
        <div className="flex xs:flex-row gap-2 font-orbitron w-full sm:w-auto">
          <button className="text-accent border-2 border-accent/50 px-6 sm:px-8 py-2 sm:py-1.5 text-xs sm:text-sm font-medium hover:scale-105 transition hover:bg-accent/10 hover:border-accent w-full sm:w-auto">
            DETAIL
          </button>
